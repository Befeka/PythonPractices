...
LOCAL_FILE_ADDRESS : str = "contacts_addresses.json"
...
def parse_pd_tsv(file_path : str , contacts : List[Dict]) -> None:
    """
    ...
    try :
        ...
        sys.stderr.write("tsv FILE NOT FOUND !!!")
        sys.exit(1)
        
    contact_dict = dict()
    #replace any nan value  with string "nan"
    df.fillna("nan",inplace=True)
    
    #convert dataframe to dictionary
    df_dict = df.to_dict(orient='records')
    
    # traverse through each row and parse as a dictionary
    for cont in df.index:
        contact_dict = dict()
        contct = df_dict[cont]
        full_name = "" # concatenate full name , start , middle and last keys to name 
        zip_val = ""
        for key , val in contct.items():
            ...
            if val != "nan":
                ...
                if key.startswith("first") or key.startswith("middle") or key.startswith("last"):
                    if "N/M/N" in str(val):
                        continue
                    full_name += " " + str(val)
                    contact_dict['name']= full_name

                if key.startswith("zip"):
                    key = "zip"
                    zip_val +=  str(val) + "-"
                    contact_dict[key] = zip_val[:-1]
                else:
                    contact_dict[key] = val
                
                
                         
        # append each row(contact address) to global contacts list
   ...
   # traverse  and parse  each input files 
    for f in files_name:
        ...
        if f.endswith(".xml"):
            ...
            if fun_type ==1:
                parse_pd_tsv(file_path,contacts)
                
            else:
                parse_input_tsv(file_path,contacts)  
                       
        elif  f.endswith(".txt"):
                
            parse_input_txt(file_path,contacts)
                
        else:
                return {}

def write_json( contacts : List[Dict],local_file_address : str = None,) ->None:
    ...
    if local_file_address != None:
        with open(local_file_address, "w") as file:
...
def main(local_file_address):
    ...
    parser.add_argument('input3.txt',nargs='?', help='input text file')
    ...
    write_json(CONTACT_PROFILE,local_file_address)
    

    
if __name__=="__main__":
    main(LOCAL_FILE_ADDRESS )